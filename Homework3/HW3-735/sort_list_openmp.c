...
//
void thread_sort_task(void *parallel_sort_task_data) {
    ...
    ptr = ((struct omp_task_args *)(parallel_sort_task_data))->ptr;
    my_id = ((struct omp_task_args *)(parallel_sort_task_data))->i;
    np = ((struct omp_task_args *)(parallel_sort_task_data))->np;
    num_threads = ((struct omp_task_args *)(parallel_sort_task_data))->task_threads;
...
void omp_sort_list() {
    ...
    #pragma omp parallel num_threads(num_threads) default(shared)
    {
        #pragma omp for
        for (int i = 0; i < num_threads; i++) {
            struct omp_task_args parallel_task_data;
            parallel_task_data.i = i;
            parallel_task_data.np = np;
            parallel_task_data.task_threads = num_threads;
            parallel_task_data.ptr = ptr;
            thread_sort_task((void *)(&parallel_task_data));
        }
    }
}
